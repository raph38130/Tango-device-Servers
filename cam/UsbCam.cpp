...
extern "C" void deviceOpen(int n)  ;
...
//  img       |  Tango::DevUChar	Image  ( max = 176 x 144)
//================================================================
...
void UsbCam::delete_device()
{
  ...
  captureStop(fd);
  deviceUninit(buffers,n_buffers);
  deviceClose(fd);
...
void UsbCam::init_device()
{
	...
	//	Initialize device
	deviceOpen(deviceName,fd,width,height);
	deviceInit(fd,deviceName,buffers,n_buffers,width,height);
	captureStart(fd,buffers,n_buffers);
	...
	attr_img_read = new Tango::DevUChar[176*144];
...
{
	DEBUG_STREAM << "UsbCam::always_executed_hook()  " << device_name << endl;
...
void UsbCam::read_img(Tango::Attribute &attr)
{
	...
	mainLoop(deviceName,fd,buffers,n_buffers,attr_img_read,width,height);
	attr.set_value(attr_img_read, width, height);
